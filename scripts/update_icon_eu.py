...
def step_hours(value):
    ...
    if not match: raise ValueError(f'Unknown GRIB time unit: {value}')
    hours = float(match[1]) * {'s':1/3600, 'm':1/60, 'h':1, 'd':24, '':1}[match[2]]
    if not hours.is_integer(): raise ValueError(f'Non-hourly ICON-EU step: {value}')
    return int(hours)
...
def read_field(url, run, step, variable, communes):
    ...
    if payload:
        ...
        try:
            ...
            if actual_run != run or step_hours(codes_get(handle, 'endStep')) != step:
            ...
            start = step_hours(codes_get(handle, 'startStep'))
...
def to_hourly(raw, starts, steps=STEPS):
    ...
    gust = np.full((121, raw['vmax_10m'].shape[1]), np.nan); periods = [None] * 121
    previous = -1
    for i, end in enumerate(steps):
        first = max(previous + 1, starts[i] + 1)
        gust[first:end+1] = raw['vmax_10m'][i]
        periods[first:end+1] = [max(1, end - starts[i])] * max(0, end + 1 - first)
        previous = end
    ...
    if not np.all(np.isfinite(result[:,:,[0,1,2,4]])): raise ValueError('Non-finite output')
    return np.round(result, 1), periods[1:]
...
def generate(catalog, output, repository, force=False):
    ...
    # Validate the actual files at both horizon boundaries before queuing all downloads.
    checked = set()
    for v in VARIABLES:
        for i in (0, len(STEPS)-1):
            raw[v][i], start = read_field(listings[v][(run,STEPS[i])],run,STEPS[i],v,communes)
            if v == 'vmax_10m': starts[i] = start
            checked.add((v,i))
        print(f'{v}: précontrôle GRIB OK', flush=True)
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        jobs = {pool.submit(read_field,listings[v][(run,s)],run,s,v,communes):(v,i) for v in VARIABLES for i,s in enumerate(STEPS) if (v,i) not in checked}
        for count, future in enumerate(concurrent.futures.as_completed(jobs),1):
            v,i = jobs[future]
            try: field,start = future.result()
            except Exception:
    ...
    serialized = hourly.astype(object)
    serialized[~np.isfinite(hourly)] = None
