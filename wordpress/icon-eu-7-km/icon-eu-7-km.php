...
add_shortcode('icon_eu_meteo', function ($atts) {
    ...
    wp_enqueue_style('icon-eu-7-km', plugins_url('assets/icon-eu.css', __FILE__), array(), '3.0.0');
    wp_enqueue_script('icon-eu-7-km', plugins_url('assets/icon-eu.js', __FILE__), array(), '3.0.0', true);
    ...
    <section class="icon-eu-widget" data-code="<?php echo esc_attr($code); ?>" data-source="https://raw.githubusercontent.com/alertesmeteo-hub/ICON-EU-7-km/data/">
      ...
      <nav class="icon-eu-tabs" aria-label="Type de prévision ICON-EU">
        <button type="button" data-icon-view="fixed" aria-pressed="true">Europe/France</button>
        <button type="button" data-icon-view="france">France Zoom interactif</button>
        <button type="button" data-icon-view="europe">Europe Zoom interactif</button>
